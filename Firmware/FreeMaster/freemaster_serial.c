...
static void FMSTR_Process_OS_IO(void)
{
  ...
  if (!fms_flags.flg.tx_active)
  {
    ...
    do
    {
      if (Virtual_com2_get_data(&rx_char, 2) == RESULT_OK) if (FMSTR_Rx(rx_char)) break;
    }
    while (1);
  }
  ...
  if (fms_flags.flg.tx_active)
  {
    ...
    while (1)
    {
      if (FMSTR_Tx((uint8_t *)&tx_char) == FMSTR_TRUE)
      {
        ...
        if (cnt > 0) Virtual_com2_send_data_sync(out_buff, cnt, 10);
        break;
      }
      ...
      if (cnt >= FREEM_TX_BUFF_SZ)
      {
        ...
        Virtual_com2_send_data_sync(out_buff, cnt, 10);
        cnt = 0;
      }
    }
  }
