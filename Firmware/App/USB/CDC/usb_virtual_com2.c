...
usb_status_t USB_DeviceCdcVcom2Callback(class_handle_t class_handle, uint32_t event, void *param)
{
  ...
  uint32_t                                           len;
  uint16_t                                           *uartBitmap;
  T_usb_cdc_acm_info                                 *cdc_info = &vcom2_cbl.g_usb_cdc_acm_info;
  ...
  case kUSB_DeviceCdcEventSendResponse:
    {
      if ((ep_message->length != 0) && (!(ep_message->length % g_cdcVcom2DicEndpoints[0].maxPacketSize)))
      {
        /* If the last packet is the size of endpoint, then send also zero-ended packet,
         ** meaning that we want to inform the host that we do not have any additional
         ** data, so it can flush the output.
         */
        USB_DeviceCdcAcmSend(class_handle, CDC_VCOM2_DIC_BULK_IN_ENDPOINT, NULL, 0);
      }
      else if (1 == g_composite.attach) //&& (1 == vcom2_cbl.start_transactions))
      {
        if ((ep_message->buffer != NULL) || ((ep_message->buffer == NULL) && (ep_message->length == 0)))
        {
          ...
          //
          //ITM_EVENT8(1, 4);
          OSFlagPost(&vcom2_cbl.os_flags, VCOM_FLAG_TRANSMITED, OS_OPT_POST_FLAG_SET, &err);

          // Подготоавливаем буфер конечной точки для приема
          // Считая что задача здесь всегда готова к приему
          //USB_DeviceCdcAcmRecv(class_handle, USB_CDC_VCOM_BULK_OUT_ENDPOINT, usb_rx_buff, g_cdcVcom2DicEndpoints[0].maxPacketSize);
  ...
  case kUSB_DeviceCdcEventRecvResponse:
    {
      if (g_composite.attach == 1) // && (vcom2_cbl.start_transactions == 1))
      {
        uint32_t    headi =  vcom2_cbl.rx_head;;

        // Пакет принят
        vcom2_cbl.rx_pack_cbl[headi].len = ep_message->length;
        vcom2_cbl.rx_pack_cbl[headi].pos = 0;

        //  Перевести указатель на следующий свободный для приема буфер
        headi++;
        if (headi >= IN_BUF_QUANTITY) headi = 0;

        if (headi != vcom2_cbl.rx_tail)
        {
          // Если есть еще свободный буффер в очереди то назначить прием в него
          vcom2_cbl.rx_head = headi;
          if  (USB_DeviceCdcAcmRecv(class_handle, CDC_VCOM2_DIC_BULK_OUT_ENDPOINT, vcom2_cbl.rx_pack_cbl[vcom2_cbl.rx_head].buff, g_cdcVcom2DicEndpoints[0].maxPacketSize) != kStatus_USB_Success) vcom2_cbl.rx_errors++;
        }
        else
        {
        ...
        OSFlagPost(&vcom2_cbl.os_flags, VCOM_FLAG_RECEIVED, OS_OPT_POST_FLAG_SET, &err);
      }
    }
    break;
  ...
  case kUSB_DeviceCdcEventSetControlLineState:
    {
      vcom2_cbl.g_usb_cdc_acm_info.dteStatus = cdc_request->setupValue;
      ...
      *uartBitmap = cdc_info->uartState;
      ...
      {
        error = USB_DeviceCdcAcmSend(class_handle, CDC_VCOM2_CIC_INTERRUPT_IN_ENDPOINT, cdc_info->serialStateBuf, len);
      ...
      if (cdc_info->dteStatus & USB_DEVICE_CDC_CONTROL_SIG_BITMAP_DTE_PRESENCE)
      {
        ...
        {
          OS_ERR err;
          vcom2_cbl.start_transactions = 1;
          OSFlagPost(&vcom2_cbl.os_flags, VCOM_FLAG_TRANSMITED, OS_OPT_POST_FLAG_SET, &err);  // Объявляем готовность к передаче
        }
      }
      else
      {
        ...
        if (1 == g_composite.attach)
        {
          vcom2_cbl.start_transactions = 0;
          //OSFlagPost(&vcom2_cbl.os_flags, VCOM_FLAG_TRANSMITED, OS_OPT_POST_FLAG_CLR, &err);  // Объявляем готовность к передаче
        }
      }
    }
    break;
...
{
  OS_ERR   err;
  OSFlagPend(&vcom2_cbl.os_flags, (OS_FLAGS)VCOM_FLAG_TRANSMITED, ticks, OS_OPT_PEND_FLAG_SET_ANY, (CPU_TS *)0U, &err);
  if (err != OS_ERR_NONE)
  {
    return RESULT_ERROR;
  }
...
/*------------------------------------------------------------------------------
 Посылка буфера данных асинхронная 
...
uint32_t Virtual_com2_send_data(uint8_t *buff, uint32_t size, uint32_t ticks)
{
  ...
  do
  {
    if (ticks != 0)
    {
      OSFlagPend(&vcom2_cbl.os_flags, (OS_FLAGS)VCOM_FLAG_TRANSMITED, ticks, OS_OPT_PEND_FLAG_SET_ANY + OS_OPT_PEND_FLAG_CONSUME, (CPU_TS *)0U, &err);
      if (err != OS_ERR_NONE)
      {
        vcom2_cbl.tx_errors++;
        return RESULT_ERROR;
    ...
    if (kStatus_USB_Success != USB_DeviceCdcAcmSend(vcom2_cbl.class_handle, CDC_VCOM2_DIC_BULK_IN_ENDPOINT, buff, sz))
    {
      vcom2_cbl.tx_errors++;
      return RESULT_ERROR;
    }
...
/*------------------------------------------------------------------------------
 Посылка буфера данных синхронная 
...
uint32_t Virtual_com2_get_data(uint8_t *b, uint32_t ticks)
{
  ...
  if (taili == vcom2_cbl.rx_head)
  {
    if (ticks != 0)
    {
      OSFlagPend(&vcom2_cbl.os_flags, (OS_FLAGS)VCOM_FLAG_RECEIVED, ticks, OS_OPT_PEND_FLAG_SET_ANY + OS_OPT_PEND_FLAG_CONSUME, (CPU_TS *)0U, &err);
      if (err != OS_ERR_NONE)
      {
        return RESULT_ERROR;
      }
  ...
  *b = vcom2_cbl.rx_pack_cbl[taili].buff[vcom2_cbl.rx_pack_cbl[taili].pos];
  ...
  if (vcom2_cbl.rx_pack_cbl[taili].pos >= vcom2_cbl.rx_pack_cbl[taili].len)
  {
    ...
    if (vcom2_cbl.rx_full == 1) // Если очередь была заполнена, то посылаем запрос приема , так как в обработчике собыйтий запрос приема был пропущен
    {
      ...
      res =  USB_DeviceCdcAcmRecv(vcom2_cbl.class_handle,  CDC_VCOM2_DIC_BULK_OUT_ENDPOINT, vcom2_cbl.rx_pack_cbl[vcom2_cbl.rx_head].buff, g_cdcVcom2DicEndpoints[0].maxPacketSize);
      if (res != kStatus_USB_Success) vcom2_cbl.rx_errors++;
    }
  }
