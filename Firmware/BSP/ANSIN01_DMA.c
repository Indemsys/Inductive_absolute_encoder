...
/*-------------------------------------------------------------------------------------------------------------
  Инициализируем DMA для передачи 32-и битных данных из SPI
 
  Передача по DMA состоит из минорных и мажорных циклов.
  После каждого запроса от периферии всегда выполняется полное количество минорных циклов и один мажорный, после чего запрос периферии сбрасывается
-------------------------------------------------------------------------------------------------------------*/
void Config_DMA_for_SPI_TX(T_DMA_SPI_TX_config *cfg, T_DMA_SPI_cbl *pDS_cbl)
{
  ...
  DMA->TCD[cfg->ch].ATTR = 0
    ...
    + LSHIFT(_Get_attr_size(cfg->minor_tranf_sz), 8)  // SSIZE | 32-x битная пересылка из источника
    + LSHIFT(0, 3)  // DMOD  | Модуль адреса приемника
    + LSHIFT(_Get_attr_size(cfg->minor_tranf_sz), 0)  // DSIZE | 32-x битная пересылка в приемник
  ;
  DMA->TCD[cfg->ch].CSR = 0
    ...
    + LSHIFT(0, 2)  // INTHALF     | Enable an interrupt when major counter is half complete
    + LSHIFT(cfg->int_en, 1)  // INTMAJOR    | Не используем прерывание по окнчании пересылки DMA
    + LSHIFT(0, 0)  // START       | Channel Start. If this flag is set, the channel is requesting service.
  ;
...
void Config_DMA_for_SPI_RX(T_DMA_SPI_RX_config *cfg, T_DMA_SPI_cbl *pDS_cbl)
{
  ...
  DMA->TCD[cfg->ch].CSR = 0
    ...
    + LSHIFT(1, 3)  // DREQ        | Disable Request. If this flag is set, the eDMA hardware automatically clears the corresponding ERQ bit when the current major iteration count reaches zero.
    + LSHIFT(0, 2)  // INTHALF     | Enable an interrupt when major counter is half complete
    + LSHIFT(cfg->int_en, 1)  // INTMAJOR    | Используем прерывание по окончании пересылки DMA
    + LSHIFT(0, 0)  // START       | Channel Start. If this flag is set, the channel is requesting service.
  ;
...
/*-----------------------------------------------------------------------------------------------------
 ...
 \param DMA_MUX - указатель на мультиплексор 
 \param mux_ch  - номер входа у модуля мультиплексора
 \param dma_ch  - номер входа у модуля DMA 
-----------------------------------------------------------------------------------------------------*/
void Set_DMA_MUX_chanel(DMAMUX_Type  *DMA_MUX, uint32_t mux_ch, uint32_t dma_ch)
{
  DMA_MUX->CHCFG[dma_ch] = mux_ch + BIT(7); // Через мультиплексор связываем сигнал от внешней периферии с входом выбранного канала DMA
}
