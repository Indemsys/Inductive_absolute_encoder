#ifndef __ANSIN01_SPI
  #define __ANSIN01_SPI
...
typedef struct
{
   OS_FLAG_GRP        spi_flags; // При использовании RTOS
   uint32_t           tx_err_cnt;
   uint32_t           rx_err_cnt;
...
#ifdef _SPI_GLOBAL_
const T_SPI_modules spi_mods[2] =
{
  { SPI0, SPI0_IRQn, LDC1101_DMA_SPI_PRIO},
  { SPI1, SPI1_IRQn, HOST_DMA_SPI_PRIO   },
};

T_SPI_errors  spi_errors[2];
T_SPI_cbls    spi_cbl[2];

#else

extern const T_SPI_modules spi_mods[2];
extern T_SPI_errors  spi_errors[2];
extern T_SPI_cbls    spi_cbl[2];
